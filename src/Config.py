...
class Config:
  def __init__(self):
    ...
    parser.add_argument('--hpo', required=False, default="None", type=str, help='HPO algorithm to apply: none | bayesian | grid | multiarmed | random | hyperband')
    ...
    parser.add_argument('--search_strategy', required=False, type=str, help='The search strategy to use: UserDefined | FreeSearch | [var]_[val]. `UserDefined` (just MiniZinc) indicates we use the search strategy in the model, if any. `free` means free search. You can use `var_` or `_val` for using the default variable selection strategy (in MiniZinc it is indomain_min and domwdeg, in XCSP3 it chosen by the solver). Only when --hpo is `None`.')
    parser.add_argument('--hyperparameters_restart', required=False, default="None", type=str, help='The restart hyperparameters the algorithm will optimise: None | Restart | Full_Restart (Restart only optimize the restart method while Full_Restart also try to optimise the coefficients of the restart method).')
    parser.add_argument('--hyperparameters_search', required=False, type=str, default="None", help='The search hyperparameters the algorithm will optimise: None | Only_Var | Only_Val | Simple_Search | Block_Search')
